...
def collate(
    ...
    ncbi_batch = [s["ncbi"] for s in samples]
    ncbi_batch = torch.IntTensor([ncbi_batch[index] for index in sort_order])

    center_batch = collate_motif("center", 0).index_select(0, sort_order)

    ligand_coor = collate_coor("ligand_coor", 0)
    ligand_coor = ligand_coor.index_select(0, sort_order)  # [B, L, 3]
    ligand_atom = collate_sub_atom("ligand_atom", 0)
    ligand_atom = ligand_atom.index_select(0, sort_order)  # [B, L, 5]
    ligand_binding = collate_label("ligand_binding")
    ligand_binding = ligand_binding.index_select(0, sort_order)
    ...
    batch = {
        ...
        "ligand_input": {"ligand_coor": ligand_coor, "ligand_atom": ligand_atom, "ligand_binding": ligand_binding},
        "pdb": pdb_batch,
        "ncbi": ncbi_batch,
        "center": center_batch,
    }
...
class NCBIDataset(FairseqDataset):
    """
    ...
    def __init__(
        ...
        self.ligand_coor = ligand_coor
        self.ligand_binding = ligand_bind
    ...
    def __getitem__(self, index):
        ...
        ncbi_item = self.ncbi[index]
        ligand_atom = self.ligand_atom[index]
        ligand_coor = self.ligand_coor[index]
        ligand_binding = self.ligand_binding[index]
